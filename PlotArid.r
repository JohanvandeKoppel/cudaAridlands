...
on=1;off=0;
setwd('/Simulations/CUDA/AridLands')
...
FID = file("build/AridLands.dat", "rb")
...
for (jj in 0:(NumFrames-1)){  # Here the time loop starts 
   ...
   if (AllWindows==on){
     ...
     mtext(text=paste("Time : ",sprintf("%1.0f",RecordTimes[jj+1]),
                      "of" ,sprintf("%1.0f",RecordTimes[NumFrames]), "days"), 
...
if (Movie==on) { 
   ...
   CmdLine=sprintf("ffmpeg -y -r 30 -i %s -s %s -c:v libx264 -pix_fmt yuv420p -b:v 5000k %s", 
